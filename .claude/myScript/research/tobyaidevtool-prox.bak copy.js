...
function promptOS() {
  return new Promise((resolve) => {
    ...
    console.log("2. macOS (Zsh)");
    console.log("3. Linux (Bash)");

    rl.question("Nhập số (1-3): ", (answer) => {
      rl.close();
      const choice = answer.trim();
      if (choice === "1") resolve("windows");
      else if (choice === "2") resolve("macos");
      else if (choice === "3") resolve("linux");
      else resolve("windows"); // default
    });
  });
}
...
function uninstallClaudeCode() {
  console.log("📦 Uninstalling Claude Code...");
  try {
    ...
    console.warn("⚠️  Could not uninstall Claude Code:", error.message);
  }
}
...
async function uninstall() {
  ...
  const confirmClaudeCode = await promptConfirm(
    "\n❓ Gỡ cài đặt Claude Code luôn không? (y/N): "
  );

  removeProXConfig();
  await removeEnvVars(osType);

  if (confirmClaudeCode === "y" || confirmClaudeCode === "yes") {
    uninstallClaudeCode();
  }
...
async function main() {
  ...
  await setEnvCommand(osType, authToken);
